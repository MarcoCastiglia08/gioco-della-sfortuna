 ...
 * @property {number} id - Identificativo univoco della carta
 * @property {string} nome - Nome dell'evento sfortunato
 * @property {number|string} immagine - Riferimento all'asset dell'immagine (require)
 * @property {number} indice - Valore numerico di sfortuna utilizzato per il confronto
 */

/**
 * Array statico contenente l'elenco completo delle carte del gioco.
 * @type {Carta[]}
 */
const CARTE = [
...
/**
 * Mescola gli elementi di un array in modo casuale.
 * @param {Array} arr - L'array da mescolare
 * @returns {Array} Un nuovo array mescolato
 */
const shuffle = (arr) => [...arr].sort(() => Math.random() - 0.5);

/**
 * Componente principale del "Gioco della Sfortuna".
 * Gestisce le fasi di gioco, il posizionamento delle carte, il punteggio (vite) e il timer di gioco.
 * @component
 */
export default function App() {
  /**
   * Stato della fase attuale del gioco ('home', 'gioco', 'round', 'risultato', 'fine').
   * @type {[string, function]}
   */
  const [fase, setFase] = useState("home");

  /**
   * Elenco delle carte attualmente nella mano del giocatore.
   * @type {[Carta[], function]}
   */
  const [mano, setMano] = useState([]);

  /**
   * La carta estratta per il round corrente che deve essere indovinata.
  ...
  /**
   * Tempo rimanente in secondi per il round corrente.
   * @type {[number, function]}
   */
  const [timer, setTimer] = useState(30);

  /**
   * Messaggio di testo da mostrare a schermo basato sull'esito delle azioni.
   * @type {[string, function]}
   */
  const [messaggio, setMessaggio] = useState("");

  /**
   * Avvia una nuova partita azzerando le vite, estraendo 3 carte iniziali ordinate per indice.
   */
  const startGame = () => {
  ...
  /**
   * Seleziona una nuova carta non presente in mano e avvia un round con timer a 30 secondi.
   */
  const nuovoRound = () => {
  ...
  /**
   * Verifica se la carta selezionata della mano ha un indice inferiore rispetto alla carta del round.
   * @param {Carta} c - La carta selezionata dall'utente per il confronto
   */
  const controlla = (c) => {
  ...
  /**
   * Effetto per la gestione del countdown del timer durante la fase di 'round'.
   */
  useEffect(() => {
  ...
  /**
   * Effetto per il controllo delle condizioni di vittoria (6 carte) o sconfitta (0 vite).
   */
  useEffect(() => {
    if (vite === 0) {
      setFase("fine");
